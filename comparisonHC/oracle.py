...
class HandlerTriplets():
    ...
    def get_AddS_comparisons(self):
        ...
        AddS_comparisons = csr_matrix((entries,(rows,columns)),shape=(self.n_examples**2,self.n_examples),dtype=int)
...
class OracleTriplets(HandlerTriplets):
    ...
    def _get_comparisons(self, n_examples, n_comparisons):
        ...
        entries = np.multiply(np.where(self.similarities[i,j] > self.similarities[i,k],1,0) + np.where(self.similarities[i,j] < self.similarities[i,k],-1,0),noise)

        columns = self._get_k(j,k)

        comparisons = coo_matrix((entries,(i,columns)),shape=(n_examples,(n_examples*(n_examples-1))//2),dtype=int)
...
class HandlerQuadruplets():
    ...
    def get_MulK_comparisons(self):
        ...
        MulK_comparisons = csr_matrix((entries,(rows,columns)),shape=(self.n_examples,self.n_examples*n_kl),dtype=int)
...
#        self.normalisation = np.zeros((n_examples,n_examples),dtype=int)
    ...
    def _get_comparisons(self, n_examples, n_comparisons):
        ...
        rows,columns = self._get_ij(np.array(rd_random_state.sample(range((n_entries*(n_entries-1))//2),n_comparisons)),n_entries)
        ...
        noise = np.where(random_state.rand(n_comparisons)<self.proportion_noise,-1,1)
        entries = np.multiply(np.where(self.similarities[i,j] > self.similarities[k,l],1,0) + np.where(self.similarities[i,j] < self.similarities[k,l],-1,0),noise)
